...
functions[2]['sport'] = function () {
    let a = 'В СУПЕР ПУПЕР БУПЕР соревновании по биатлону участвуют спортсмены из 25 стран, одна из которых ― Россия. Всего на старт вышло 60 участников, из которых 6 ― из России. Порядок старта определяется жребием, стартуют спортсмены друг за другом. Какова вероятность того, что десятым стартовал спортсмен из России?'
...
let part: string = ''
part += '15-го января планируется взять кредит в банке на '
part += N[i].toString()
...
part += (KK[i]-100).toString()
part += '% больше суммы, взятой в кредит. Найдите r.'
...
functions[15]['DPR1Q2'] = function () {
    ...
    let part: string = ''
    part += 'Абрамчик взял кредит в банке на срок '
    part += N[i].toString()
    part += ' месяцев. В конце каждого месяца общая сумма оставшегося долга увеличивается на '
    part += (RR[i]-100).toString()
    part += '%, а затем уменьшается на сумму, уплаченную Абрамчиком. Суммы, выплачиваемые в конце каждого месяца, подбираются так, чтобы в результате сумма долга каждый месяц уменьшалась равномерно, то есть на одну и ту же величину. Сколько процентов от суммы кредита составила сумма, уплаченная Абрамчиком банку сверх кредита?'
...
functions[15]['DPR4Q2'] = function () {
    ...
    let ANS = Big(KK[i]).times(Big(S)).div(Big(100)).toNumber();
...
let XXN = Big(S).times(Big(RR[i])).div(Big(N[i]));



let part: string = ''
part += `В июле Моше хочет взять кредит в банке на сумму ${S} млн рублей на некоторый срок (целое число лет). Условия его возврата таковы:\n - каждый январь долг возрастает на  ${RR[i]-100}% по сравнению с концом предыдущего года;\n - с февраля по июнь каждого года необходимо выплатить часть долга;\n - в июле каждого года долг должен быть на одну и ту же сумму меньше долга на июль предыдущего года.\nСколько Моше в итоге выплатит после полного погашения кредита, если наименьший годовой платёж составит ${XXN} млн рублей?`
let ANS = Big(KK[i]).times(Big(S)).div(Big(100)).toNumber();


return [part, ANS]
}
functions[15]['DPR4Q3'].URL = 'YOUTUBE DPR4Q3 URL'
            ...
            if (trs == null) {
                ...
                switch (trs) {
                    ...
                    case '15':
                        ...
                        await interaction.reply('```'+'ТИПЫ 15-й ЗАДАЧИ (ЧАСТЬ 1)' +
`
+--------+-------------------------------------------------------------------------+
...
| RV1Q2  | Равные выплаты. Обычная. То же, что RV1Q1, но в условии не X, а сумма X |
| RV1Q3  | Равные выплаты. Обычная. Теперь надо найти выплату X, зная S            |
| RV1Q4  | Равные выплаты. Обычная. То же, что RV1Q3, но теперь в ответ сумму X    |
| RV1Q5  | Равные выплаты. Обычная. То же, что RV1Q1, но знаем NX-S, а не просто X |
| RV2    | Равные выплаты. Прикол с пропуском платежей (они через раз). Найти X    |
| RV3REF | Равные выплаты. Найти экономию от рефинансирования кредита (2 выплаты)  |
| RV3MIN | Равные выплаты. Найти минималку лет. Прикол в том, чтобы тупо считать   |
...
+--------+-------------------------------------------------------------------------+
` +'```' )
await interaction.followUp('```'+'ТИПЫ 15-й ЗАДАЧИ (ЧАСТЬ 2)' +
`
+--------+-------------------------------------------------------------------------+
| КОД    | ОПИСАНИЕ ПОДТИПА ЗАДАЧИ                                                 |
+--------+-------------------------------------------------------------------------+
| DPR1Q1 | Долг падает равномерно. Дана пропорция сумма выплат/кредит. Найти r     |
| DPR1Q2 | Долг падает равномерно. Дано r. Найти пропорцию переплата/кредит        |
| DPR1Q3 | Долг падает равномерно. Дано r. Найти пропорцию сумма выплат/кредит     |
| DPR1Q4 | Долг падает равномерно. Дано r и переплата/кредит. Найти N              |
